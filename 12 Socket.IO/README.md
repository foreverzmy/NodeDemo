...
第一个目标是建立一个简单的HTML页面，提供一个表单和一个消息列表，我们将使用Node.js的web框架`express`完成这个目标，请确保Node.js已安装。

首先创建一个`package.json`清单文件来描述项目，建议把它放在一个专门的空目录。

> 注：在我的仓库中，我命名为`socket-chat-example`，但是我把这个项目放在了01文件夹中，02...是对该项目的扩充。
...
现在，为了便于安装我们所需要的依赖(相当于jar包)，我们会使用`npm install --save`：
...
在用户键入消息时，我们让服务端接收并作为一个`chat message`事件，`index.html`的`script`部分应如下所示：
...
、[我的DEMO](https://github.com/zmyforever1/NodeDemo/tree/master/12%20Socket.IO)
